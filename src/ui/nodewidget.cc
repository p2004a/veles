...
void NodeWidget::showMinimap(bool show) {
  if (minimap_dock_ == nullptr) {
    ...
    connect(minimap_, &visualization::MinimapPanel::selectionChanged,
        hex_edit_widget_, &HexEditWidget::minimapSelectionChanged, Qt::QueuedConnection);
  }
...
void NodeWidget::updateMinimap() {
  ...
  if (last_update_minimap_size_ != update_minimap_size_) {
    minimap_->createMinimaps(update_minimap_size_, k_minimap_selection_factor);
    last_update_minimap_size_ = update_minimap_size_;
  }
  minimap_->scrollTo(update_minimap_start_);
}
